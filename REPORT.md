...
The CI/CD pipeline is defined in `.github/workflows/ci-cd.yml` and executed by **GitHub Actions** on every push or pull request targeting `main`/`master`. Jobs are chained with `needs:` so each stage only begins when the previous one succeeds. A failure in any job terminates the pipeline immediately, satisfying the requirement for automatic failure propagation.
...
|---|---|
| Build | 2–3 minutes (first run, cold cache ~5 min) |
| Test | 4–6 minutes (config-server startup adds ~15 s) |
| Code Quality Analysis | 3–5 minutes (SonarCloud analysis + network) |
| Package & Docker Build | 3–5 minutes (JAR packaging + image push) |
| **Total** | **~12–19 minutes** |
...
The pipeline is **fully automated** from commit to container image. No manual intervention is required between a `git push` and a published image on GHCR. Test execution, coverage measurement, quality gate evaluation, packaging, and Docker push all occur without human input. The only intentional manual step is the deployment of the Docker Compose stack on a target machine, which was kept manual to avoid coupling the pipeline to a specific deployment environment.
...
2. **Introduce a DAST / smoke-test stage.** After the Docker image is pushed, a fifth stage could use `docker-compose up -d` on the runner, wait for health checks to pass, and execute a small suite of HTTP smoke tests (e.g. with `httpie` or `curl`). This would close the gap between "image published" and "image proven to run", adding end-to-end deployment confidence to the pipeline.
...
**Assignment Branch:** `CICD_Assignment1_Finished`  
**Default Branch:** `master`  

The repository is publicly accessible. The pipeline definition is at:  
`.github/workflows/ci-cd.yml`

All changes described in this report correspond to commits on the `master` and `CICD_Assignment1_Finished` branches. Key commit messages include:
